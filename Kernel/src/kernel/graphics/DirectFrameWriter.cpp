...
{
    const uint8_t* glyph = get_char_bitmap(c);
    if (glyph == nullptr) return KERNEL_FAILURE;

    int outSize = (int)floor_f(CHARACTOR_BITMAP_HEIGHT * scale);
    if (outSize <= 0) return KERNEL_FAILURE;
    ...
    for (int dy = 0; dy < outSize; dy++)
    {
        ...
        for (int dx = 0; dx < outSize; dx++)
        {
            ...
            if (flag) k_dfw_draw_pixel({ point.x + dx, point.y + dy }, color);
        }
    }
...
KERNEL_STATUS k_dfw_draw_string_f(POINT point, float scale, COLOR color, const char* fmt, ...)
{
    ...
    uint64_t x = point.x;
    int stride = (int)floor_f(CHARACTOR_BITMAP_HEIGHT * scale);
    if (stride <= 0) stride = 1;

    for (const char* p = buffer; *p != '\0'; p++)
    {
        k_dfw_draw_char({ x, point.y }, scale, color, *p);
        x += stride;
    }
